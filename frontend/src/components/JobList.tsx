      ...
      {jobList.map(j => {
        ...
        return (
          <div key={j.job_id} id={`log-${j.job_id}`} className="grid grid-cols-2 md:grid-cols-[1fr_0.6fr_1fr_1.5fr_0.8fr_1fr_1.6fr] gap-3 px-3 py-3 border-b border-white/[0.04] text-xs items-center">
            ...
            <span className={STATUS_CLS[status] ?? "text-ink-mute"}>{STATUS_LABEL[status] ?? status}</span>
            <span className="text-ink-mute text-[11px] hidden md:inline">{last}</span>
            <span className="flex gap-1 col-span-2 md:col-span-1">
              <button onClick={() => onTrigger(j.job_id)} className="text-[11px] px-2 py-1 rounded bg-line hover:bg-line-mid">▶</button>
              <button onClick={() => onEdit(j.job_id, j.cron_expr)} className="text-[11px] px-2 py-1 rounded bg-line hover:bg-line-mid">✏</button>
              <button onClick={() => onShowLog(j.job_id)} className="text-[11px] px-2 py-1 rounded bg-line hover:bg-line-mid">📜</button>
            </span>
          </div>
        );
      })}
